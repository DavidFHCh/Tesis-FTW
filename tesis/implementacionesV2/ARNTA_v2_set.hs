...
rbinsert x (Zero t) = blacken(ins x t)

blacken :: PRed t a -> RBT t a
blacken (C u) = Zero u
blacken (R(a,x,b)) = Suc(Zero(B(C a,x,C b)))
...
--appred :: Append t => PRed t a -> PRed t a -> RR t a
...
rbdelete x (Zero t) = blacken2 (del x t)

blacken2 :: RR t a -> RBT t a
blacken2 (C(C t)) = Zero t
blacken2 (C(R(a,x,b))) = Suc(Zero(B(C a,x,C b)))
blacken2 (R p) = Suc(Zero(B p))
